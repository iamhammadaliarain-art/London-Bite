...
export function CustomerLandingV2() {
  ...
  return <main className="min-h-screen overflow-hidden bg-black text-white">
    ...
    <header className={`sticky top-3 z-40 mx-auto flex w-[calc(100%-24px)] max-w-[1500px] items-center justify-between rounded-[24px] px-3 py-2.5 sm:px-4 ${glass}`}>
      <Link href={referral ? `/?ref=${encodeURIComponent(referral)}` : "/"} className="flex items-center gap-3 no-underline"><span className="grid size-12 place-items-center overflow-hidden rounded-[14px] bg-black shadow-sm ring-1 ring-white/10"><Image src="/brand/london-bite-official.svg" alt="London Bite" width={48} height={48} className="h-full w-full object-contain" /></span><span className="hidden leading-tight sm:grid"><strong className="text-sm text-white">London Bite</strong><small className="text-[9px] font-semibold uppercase tracking-[0.12em] text-white/48">Every Bite is a London Story</small></span></Link>
    ...
    <section className="relative mx-auto grid min-h-[720px] max-w-[1500px] items-center gap-7 px-5 py-12 lg:grid-cols-[0.92fr_1.08fr] lg:px-8 lg:py-16">
      ...
      <div className="relative min-h-[500px] lg:min-h-[640px]"><div className={`absolute inset-0 overflow-hidden rounded-[42px] p-2 ring-1 ring-lb-blue/25 ${glass}`}><Image src={popular[1]?.image ?? customerProducts[0].image} alt="London Bite food" fill priority sizes="(max-width: 1024px) 100vw, 54vw" className="rounded-[36px] object-cover p-2"/><div className="absolute inset-2 rounded-[36px] bg-gradient-to-t from-black via-black/15 to-transparent"/><div className="absolute bottom-7 left-7 right-7 text-white"><span className="text-[10px] font-black uppercase tracking-[0.16em] text-lb-red">London Bite favourite</span><strong className="mt-2 block text-2xl font-black">{popular[1]?.name ?? "Signature favourites"}</strong></div></div></div>
    </section>

    <section className="relative mx-auto max-w-[1500px] px-5 py-10 lg:px-8"><div className="mb-5 flex items-end justify-between gap-4"><div><span className="text-[10px] font-black uppercase tracking-[0.16em] text-lb-red">Popular right now</span><h2 className="mb-0 mt-2 text-3xl font-black tracking-[-0.04em] text-white sm:text-4xl">Fast path to the craving.</h2></div><Link href={orderHref} className="hidden text-xs font-black text-[#b9b8ff] no-underline sm:block">See full menu →</Link></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{popular.map((product) => <article key={product.slug} className={`rounded-[28px] p-2.5 ${glass}`}><Image src={product.image} alt={product.name} width={960} height={720} sizes="(max-width: 640px) 100vw, (max-width: 1280px) 50vw, 25vw" className="aspect-[4/3] w-full rounded-[22px] object-cover"/><div className="p-3"><div className="flex items-start justify-between gap-3"><div><span className="text-[9px] font-black uppercase tracking-[0.14em] text-lb-red">{product.category}</span><h3 className="mb-0 mt-1 text-base font-black text-white">{product.name}</h3></div><strong className="whitespace-nowrap text-sm text-white">{money(product.price)}</strong></div><p className="mb-3 mt-2 line-clamp-2 text-xs leading-5 text-white/52">{product.description}</p><button type="button" onClick={() => fastOrder(product.slug)} className={`${primary} w-full min-h-10 px-3 text-xs`}>Add & order</button></div></article>)}</div></section>
