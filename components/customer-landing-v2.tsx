...
export function CustomerLandingV2() {
  ...
  return <main className="min-h-screen overflow-hidden bg-[#f4f5f2] text-lb-ink">
    ...
    <section className="mx-auto max-w-[1500px] px-5 py-10 lg:px-8"><div className="mb-5 flex items-end justify-between gap-4"><div><span className="text-[10px] font-black uppercase tracking-[0.16em] text-lb-red">Popular right now</span><h2 className="mb-0 mt-2 text-3xl font-black tracking-[-0.04em] text-lb-navy sm:text-4xl">Fast path to the craving.</h2></div><Link href={orderHref} className="hidden text-xs font-black text-lb-navy no-underline sm:block">See full menu →</Link></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{popular.map((product) => <article key={product.slug} className={`rounded-[28px] p-2.5 ${glass}`}><img src={product.image} alt={product.name} className="aspect-[4/3] w-full rounded-[22px] object-cover"/><div className="p-3"><div className="flex items-start justify-between gap-3"><div><span className="text-[9px] font-black uppercase tracking-[0.14em] text-lb-red">{product.category}</span><h3 className="mb-0 mt-1 text-base font-black text-lb-navy">{product.name}</h3></div><strong className="whitespace-nowrap text-sm text-lb-navy">{money(product.price)}</strong></div><p className="mb-3 mt-2 line-clamp-2 text-xs leading-5 text-lb-muted">{product.description}</p><button type="button" onClick={() => fastOrder(product.slug)} className={`${primary} w-full min-h-10 px-3 text-xs`}>Add & order</button></div></article>)}</div></section>
