...
export function CustomerOrderingV2() {
  ...
  useEffect(() => {
    ...
    void getLiveMenu().then((items) => {
      ...
      setProducts(mapped);
      setMenuNotice(mapped.length ? "Live menu · server-priced" : "Menu is temporarily unavailable");
    }).catch(() => setMenuNotice("Using cached menu · server validates availability at checkout"));
  }, []);
